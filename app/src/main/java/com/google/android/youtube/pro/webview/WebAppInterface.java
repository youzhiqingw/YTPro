...
public class WebAppInterface {
	...
	// 改用 PixelCopy 直接截 WebView 画面，绕开跨域限制。
	@JavascriptInterface
	public void captureScreenshot(final String name) {
		activity.runOnUiThread(() -> {
			try {
				final int w = web.getWidth();
				final int h = web.getHeight();
				if (w <= 0 || h <= 0) {
					Toast.makeText(activity, "Screenshot failed: webview not ready", Toast.LENGTH_SHORT).show();
					return;
				}
				if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
					...
					android.graphics.Rect rect = new android.graphics.Rect(loc[0], loc[1], loc[0] + w, loc[1] + h);
					android.view.PixelCopy.request(activity.getWindow(), rect, bmp, copyResult -> {
						if (copyResult == android.view.PixelCopy.SUCCESS) {
							persistBitmap(bmp, name);
						} else {
							try { bmp.recycle(); } catch (Exception ignored) {}
							fallbackDrawAndPersist(name, w, h);
						}
				...
				Toast.makeText(activity, "Screenshot failed: " + e.getMessage(), Toast.LENGTH_SHORT).show();
			}
		});
	}

	// API < 26（无 PixelCopy）回退：把 WebView 画进位图。硬件加速下视频区域可能黑，仅老设备兜底。
	private void fallbackDrawAndPersist(final String name, int w, int h) {
		try {
	...
	private void persistBitmap(final android.graphics.Bitmap bmp, final String name) {
		new Thread(() -> {
			try {
				java.io.ByteArrayOutputStream baos = new java.io.ByteArrayOutputStream();
				bmp.compress(android.graphics.Bitmap.CompressFormat.JPEG, 95, baos);
				String saved = storeImage(name, baos.toByteArray());
				activity.runOnUiThread(() -> Toast.makeText(activity, "Screenshot saved: " + saved, Toast.LENGTH_SHORT).show());
			} catch (SecurityException e) {
				activity.runOnUiThread(() -> Toast.makeText(activity, R.string.grant_storage, Toast.LENGTH_SHORT).show());
			} catch (final Exception e) {
				activity.runOnUiThread(() -> Toast.makeText(activity, "Screenshot failed: " + e.getMessage(), Toast.LENGTH_SHORT).show());
			} finally {
				try { bmp.recycle(); } catch (Exception ignored) {}
			}
		}).start();
